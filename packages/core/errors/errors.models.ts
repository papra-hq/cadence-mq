...
export async function safely<T>(fn: Promise<T>): Promise<[Error, undefined] | [undefined, T]> {
  return fn
    .then((result) => [undefined, result] as [undefined, T])
    .catch((error: unknown) => [castError(error), undefined] as [Error, undefined]);
}

export class CadenceError extends Error {
  override readonly cause?: unknown;
  readonly isCadenceError = true;
...
export function createErrorFactory(factorySettings: {
  code: string;
  message: string;
}): (instanceSettings?: { cause?: unknown; message?: string; code?: string }) => CadenceError {
  return (instanceSettings: { cause?: unknown; message?: string; code?: string } = {}) =>
    createError({
